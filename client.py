...
#from kodlama import playfair_sifrele, anahtar
from sifreleme import playfair_sifrele,playfair_desifrele
from playfairdeneme import key as anahtar
...
def mesaj_gonder():
    mesaj = m_textbox.get()
    mesajsifreli = playfair_sifrele(mesaj,anahtar)
    mesaj = playfair_desifrele(mesajsifreli,anahtar)
    if mesaj != '':
...
def serverden_mesaji_al(kullanici):
    ...
    while 1:
        ...
        if mesaj != '':
            kullaniciadi = mesaj.split("~")[0]
            icerik = mesaj.split('~')[1]

            mesaj_ekle(f"[{kullaniciadi}] {icerik}")

        else:
            messagebox.showerror("Hata","Kullaniciden alinan mesaj boştur.")
